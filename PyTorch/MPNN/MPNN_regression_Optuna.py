...
def load_best_model():
    ...
    model = MPNNPredictor(node_in_feats=node_in_feats, edge_in_feats=edge_in_feats,
                        ...
                        num_step_message_passing=best_params['num_step_message_passing'], n_tasks=1,
...
print('      ***  PyTorch for regression tasks (Message Passing Neural Networks, MPNN) started at {0}  ***\n'.format(start_date.strftime("%Y-%m-%d %H:%M:%S")))
...
print('\n      ***  PyTorch for regression tasks (Message Passing Neural Networks, MPNN) terminated at {0}  ***\n'.format(end_date.strftime("%Y-%m-%d %H:%M:%S")))
if torch.cuda.is_available():
